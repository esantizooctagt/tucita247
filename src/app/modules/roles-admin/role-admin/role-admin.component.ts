...
export class RoleAdminComponent implements OnInit {
  ...
  onDisplay(){
    ...
    if (this.roleDataList != undefined && this.roleDataList != "0") {
      ...
      this.g.clear();
      this.role$ = this.adminService.getRole(this.roleDataList, this.businessId).pipe(
        tap(res => {
          if (res != null) {
  ...
  onSubmit(){
    ...
    if (this.roleForm.touched){
      ...
      if (roleId !== '' && roleId !== null) {
        ...
        this.roleSave$ = this.adminService.updateRole(dataForm).pipe(
          tap(res => { 
            this.spinnerService.stop(spinnerRef);
        ...
        let lines = this.roleForm.value.Access;
        dataForm['Access'] = lines.filter((s: any)=> s.Level_Access > 0);
        this.roleSave$ = this.adminService.postRole(dataForm).pipe(
          tap(res => { 
            this.spinnerService.stop(spinnerRef);
            // this.data.changeData('roles');
  ...
  loadAccess(){
    ...
    this.apps$ = this.adminService.getApplications(rolId, this.businessId, this.languageInit).pipe(
      map((res: any)=>{
        if (res != null){
